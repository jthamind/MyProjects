...
function Stop-ServiceForcefully {
    ...
    $scriptBlock = {
        ...
        Try {
            ...
            $result += "$(Get-Timestamp) - $serviceName stopped successfully."
        } Catch {
            $result += "$(Get-Timestamp) - $serviceName failed to stop."
...
if ($null -ne $EnvironmentData) {
    ...
    $OutTextControl.AppendText("$((& $TimestampFunction)) - Stop Services script: No environment data found for $Environment.`r`n")
    $ExitCode = 1
}
