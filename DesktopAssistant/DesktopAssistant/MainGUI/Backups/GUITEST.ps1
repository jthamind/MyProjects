...
# todo - CURRENT ISSUES
# todo - Currently, logic checks if theme is premium and uses accent/disabled colors if it is. Need to adjust this to include Custom colors using those same variables
# todo - For the issue with Restarts GUI, see if it's caused by multiple services being selected
# todo - "Cannot add a member with the name 'XXXXXX' because a member with that name already exists. To overwrite the member anyway, add the Force parameter to your command."
# todo - Speed up initial load time
# todo - Make all paths relative

# todo - FUTURE RELEASE FEATURES
# todo - Full documentation for this app
# todo - Module for remote PowerShell sessions on servers
# todo - Module for restarting DirectoryWatcher services
# todo - Guided walkthrough
# todo - Theme Builder
# todo - EC2 monitoring (need to see if access keys are possible)
...
$AlliedIcon = $ConfigValues.AlliedIcon.Replace("{USERPROFILE}", $UserProfilePath)
$TestingWebhookURL = $ConfigValues.TestingWebhook
$script:WorkhorseServer = $ConfigValues.WorkhorseServer

# Initializing variable for holding the selected PST Environment so it can be used across multiple event handlers
$script:PSTEnvironment = $null
...
function Set-FormIcon {
    ...
    if ($SelectedTheme -eq 'NBA' -or $SelectedTheme -eq 'NFL' -or $SelectedTheme -eq 'MLB') {
        ...
        $Form.Icon = $ConfigValues.AlliedIcon.Replace("{USERPROFILE}", $UserProfilePath)
        if ($IsLenderLFPPopupActive) {
            $script:LenderLFPPopup.Icon = $ConfigValues.AlliedIcon.Replace("{USERPROFILE}", $UserProfilePath)
        }
        if ($IsHDTStoragePopupActive) {
            $script:HDTStoragePopup.Icon = $ConfigValues.AlliedIcon.Replace("{USERPROFILE}", $UserProfilePath)
        }
        if ($IsFeedbackPopupActive) {
            $script:FeedbackForm.Icon = $ConfigValues.AlliedIcon.Replace("{USERPROFILE}", $UserProfilePath)
        }
    }
}
...
function Remove-AllBulletPoints {
    foreach ($menu in @($CustomThemes, $MLBThemes, $NBAThemes, $NFLThemes, $PremiumThemes, $TraditionalThemes)) {
...
function Update-MainTheme {
    ...
    # Update these based on how they're set elsewhere in your code
    ...
    $activeTheme = $null
    switch ($Category) {
        'Custom' { $activeTheme = $CustomThemes.DropDownItems | Where-Object { $_.Text -eq $Team } }
...
function Open-SubmitFeedbackRunspace {
    ...
    $SubmitFeedbackScript = $ConfigValuesSubmitFeedbackScript.Replace("{USERPROFILE}", $UserProfilePath)
...
Function Save-CustomTheme {
    ...
    try {
        ...
        $OutText.AppendText("$(Get-Timestamp) - Custom theme $script:NewThemeName was saved successfully.`r`n")
    } catch {
        $OutText.AppendText("$(Get-Timestamp) - An error occurred while saving the theme: $_`r`n")
    }
}
...
function Open-CreateHDTStorageRunspace {
    ...
    $CreateHDTStorageScript = $ConfigValuesCreateHDTStorageScript.Replace("{USERPROFILE}", $UserProfilePath)
...
$CustomThemes = New-Object System.Windows.Forms.ToolStripMenuItem
...
foreach ($team in $ColorTheme.Custom.PSObject.Properties) {
    ...
    $CustomThemes.DropDownItems.Add($MenuItem) | Out-Null
}
...
$MenuThemeBuilder.add_Click({
    ...
    $script:ThemeBuilderSysAdminTab.Font = [System.Drawing.Font]::new("Segoe UI", 8, [System.Drawing.FontStyle]::Regular)
    ...
    $script:ThemeBuilderAppListCombo.Font = [System.Drawing.Font]::new("Arial", 9, [System.Drawing.FontStyle]::Regular)
    ...
    $script:ThemeBuilderAppListLabel.Font = [System.Drawing.Font]::new("Arial", 9, [System.Drawing.FontStyle]::Bold)
    ...
    $ThemeBuilderServicesTab.Font = [System.Drawing.Font]::new("Arial", 9, [System.Drawing.FontStyle]::Regular)
    ...
    $script:ThemeBuilderBackColorLabel.Text = 'BackColor'
    ...
    $script:ThemeBuilderForeColorLabel.Text = 'ForeColor'
    ...
    $script:ThemeBuilderAccentColorLabel.Text = 'AccentColor'
    ...
    $script:ThemeBuilderDisabledColorLabel.Text = 'DisabledColor'
    ...
    $script:ThemeBuilderApplyThemeButton.Font = [System.Drawing.Font]::new("Arial", 8, [System.Drawing.FontStyle]::Regular)
    $script:ThemeBuilderApplyThemeButton.Enabled = $false

    # Button for applying and saving user-defined colors
    ...
    $script:ThemeBuilderSaveThemeButton.Font = [System.Drawing.Font]::new("Arial", 8, [System.Drawing.FontStyle]::Regular)
    $script:ThemeBuilderSaveThemeButton.Enabled = $false
    ...
    $script:ThemeBuilderResetThemeButton.Font = [System.Drawing.Font]::new("Arial", 8, [System.Drawing.FontStyle]::Regular)
    $script:ThemeBuilderResetThemeButton.Enabled = $false
    ...
    $script:ThemeBuilderDeleteThemesButton.Font = [System.Drawing.Font]::new("Arial", 8, [System.Drawing.FontStyle]::Regular)
    ...
    $script:ThemeBuilderSaveThemeButton.add_Click({
        ...
        $script:ThemeBuilderSaveThemePopupButton.add_Click({
            ...
            # Call the Save-CustomTheme function
            Save-CustomTheme -ThemeName $script:NewThemeName -BackColor $script:CustomBackColor -ForeColor $script:CustomForeColor -AccentColor $script:CustomAccentColor -DisabledColor $script:CustomDisabledColor -Quote $script:NewThemeQuote -ColorTheme ([ref]$ColorTheme)
            ...
            $CustomThemes.DropDownItems.Add($NewMenuItem) | Out-Null

            # Sort the dropdown items alphabetically
            $SortedItems = $CustomThemes.DropDownItems | Sort-Object { $_.Text -replace '• ', '' }
            $CustomThemes.DropDownItems.Clear()
            $CustomThemes.DropDownItems.AddRange($SortedItems)
        })
        ...
        $script:ThemeBuilderSaveAndApplyThemeButton.add_Click({
            ...
            # Sort the dropdown items alphabetically
            $SortedItems = $CustomThemes.DropDownItems | Sort-Object Text
            $CustomThemes.DropDownItems.Clear()
            $CustomThemes.DropDownItems.AddRange($SortedItems)
    ...
    $script:ThemeBuilderDeleteThemesButton.add_Click({
        ...
        $script:ThemeBuilderDeleteThemesForm.Size = New-Object System.Drawing.Size(300, 400)
        ...
        $script:ThemeBuilderThemeNameListBoxLabel.Location = New-Object System.Drawing.Point(50, 50)
        $script:ThemeBuilderThemeNameListBoxLabel.Size = New-Object System.Drawing.Size(150, 20)
        $script:ThemeBuilderThemeNameListBoxLabel.Font = [System.Drawing.Font]::new("Arial", 9, [System.Drawing.FontStyle]::Bold)
        $script:ThemeBuilderThemeNameListBoxLabel.Text = 'Theme Name'

        # List box for theme names
        $script:ThemeBuilderThemeNameListBox = New-Object System.Windows.Forms.ListBox
        $script:ThemeBuilderThemeNameListBox.Location = New-Object System.Drawing.Point(50, 70)
        ...
        $script:ThemeBuilderDeleteThemesPopupButton.Location = New-Object System.Drawing.Point(50, 300)
        ...
        # Populate the list box with custom theme names
        ...
        # Logic for changing Delete Theme button text when multiple themes are selected
        $script:ThemeBuilderThemeNameListBox.add_SelectedIndexChanged({
            if ($script:ThemeBuilderThemeNameListBox.SelectedItems.Count -gt 1) {
        ...
        $script:ThemeBuilderDeleteThemesPopupButton.add_Click({
            $script:ThemesToDelete = $script:ThemeBuilderThemeNameListBox.SelectedItems

            # Loop through the selected themes and delete them
            foreach ($Theme in $script:ThemesToDelete) {
                $ThemeToDelete = $Theme.ToString()
                if ($null -ne $ColorTheme.Custom) {
                    $ColorTheme.Custom.PSObject.Properties.Remove($ThemeToDelete)
                }
            }

            $OutText.AppendText("$(Get-Timestamp) - Successfully deleted the below custom theme(s):`r`n")
            $OutText.AppendText("$(Get-Timestamp) - $($ThemesToDelete -join "`r`n")`r`n")

            # Save the changes back to the JSON file
            $jsonString = $ColorTheme | ConvertTo-Json -Depth 5
            Set-Content -Path .\ColorThemes.json -Value $jsonString

            # Update the CustomThemes dropdown
            $CustomThemes.DropDownItems.Clear()
            foreach ($Theme in $ColorTheme.Custom.PSObject.Properties) {
                $NewMenuItem = New-Object System.Windows.Forms.ToolStripMenuItem
...
$ShowToolTipsMenu.add_Click({
    if ($ShowToolTipsMenu.Text -eq "Show Tool Tips") {
        ...
        $OutText.AppendText("Tool tips enabled.`r`n")
    }
    else {
        ...
        $OutText.AppendText("Tool tips disabled.`r`n")
    }
})
...
$MenuAboutItem.add_Click({
    ...
    $AboutMenuAlliedLogoImage = [System.Drawing.Image]::FromFile($AlliedIcon)
    ...
    $VersionLabel.Text = "Version: 1.5.0"
    ...
    $UpcomingFeaturesLabel.Text = "Upcoming Features`r`n• Theme Builder`r`n• AWS EC2 Monitoring"
    ...
    $AboutForm.Add_FormClosed({
        $global:IsAboutPopupActive = $false
    })

    $AboutForm.Controls.Add($AboutLabel)
    $AboutForm.Controls.Add($AboutMenuAlliedLogo)
...
$ReverseIPButton.add_Click({
    ...
    $ReverseIPTextBox.Text = ''
    Invoke-ReverseIPLookup -ip $ip -OutText $OutText
})
            ...
            if ($script:LenderLFPTicketTextBox.Text -match '^[A-Za-z]{3}-\d{5}$') {
                if (Test-Path $AddLenderScript){
                    try {
                        ...
                        & $AddLenderScript -LenderId $LenderId -TicketNumber $TicketNumber -Environment $Environment -OutTextControl $OutText -TimestampFunction ${function:Get-Timestamp}
                    }
                    catch {
...
$MenuColorTheme.DropDownItems.Add($CustomThemes) | Out-Null
...
if ($null -ne $ConfigValues.DefaultUserTheme -and $ConfigValues.DefaultUserTheme -ne "") {
    ...
    if ($SelectedTheme -eq 'NBA' -or $SelectedTheme -eq 'NFL' -or $SelectedTheme -eq 'MLB') {
        ...
        $IconPath = $ConfigValues.AlliedIcon.Replace("{USERPROFILE}", $UserProfilePath)
