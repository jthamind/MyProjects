...
# todo - CURRENT ISSUES
# todo - Repopulate Active Tickets list box if LFP service creates new ticket
# todo - Fix Update-PST files function. Should import files when there are none. Should update files when there are some. Should create directory if needed.

# todo - TECHNICAL DEBT
# todo - Speed up initial load time (Lazy Loading)
# todo - Make all paths relative
# todo - Add Test-Path if/then logic to directory assignments and use New-Item if needed

# todo - FUTURE RELEASE FEATURES
# todo - Allies theme text colors switch between the pride flag colors every letter (ETA mid November 2023)
...
$AlliedLogo = $ConfigValues.AlliedLogo.Replace("{USERPROFILE}", $UserProfilePath)
...
function Update-PSTFiles {
    ...
    $script:RemoteSupportToolFolderDate = Get-ChildItem $RemoteSupportTool -Exclude *.txt -Recurse | ForEach-Object {$_.LastWriteTime} | Sort-Object -Descending | Select-Object -First 1
    $script:LocalSupportToolFolderDate = Get-ChildItem $LocalSupportTool -Exclude *.txt -Recurse | ForEach-Object {$_.LastWriteTime} | Sort-Object -Descending | Select-Object -First 1

    if ($script:RemoteSupportToolFolderDate -gt $script:LocalSupportToolFolderDate) {
...
$MainFormTabControl.add_SelectedIndexChanged({
    # If the SupportTab is selected and it's not initialized
    if ($MainFormTabControl.SelectedTab -eq $SupportTab -and -not $SupportTab.Tag) {
        # Add controls to the SupportTab
    ...
    if ($MainFormTabControl.SelectedTab -eq $TicketManagerTab -and -not $TicketManagerTab.Tag) {
        # Add controls to the TicketManagerTab
