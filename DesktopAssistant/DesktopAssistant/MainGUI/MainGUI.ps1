...
# todo - Terraform Admin Tools (Hopefully someday!)
...
function Set-FormIcon {
    ...
    if ($SelectedTheme -eq 'NBA' -or $SelectedTheme -eq 'NFL' -or $SelectedTheme -eq 'MLB') {
        ...
        if (Test-Path $IconPath) {
            $TeamIcon = New-Object System.Drawing.Icon($IconPath)
            $DesktopAssistantForm.Icon = $TeamIcon
            if ($global:IsCertCheckPopupActive) {
                $script:CertCheckPopup.Icon = $TeamIcon
            }
            if ($global:IsLenderLFPPopupActive) {
                $script:LenderLFPPopup.Icon = $TeamIcon
            }
            if ($global:IsBillingRestartPopupActive) {
                $script:BillingRestartPopup.Icon = $TeamIcon
            }
            if ($global:IsHDTStoragePopupActive) {
                $script:HDTStoragePopup.Icon = $TeamIcon
            }
            if ($global:IsFeedbackPopupActive) {
                $script:FeedbackPopup.Icon = $TeamIcon
            }
            if ($global:IsThemeBuilderPopupActive) {
                $script:ThemeBuilderForm.Icon = $TeamIcon
            }
            if ($global:IsColorPickerPopupActive) {
                $script:ColorPickerPopup.Icon = $TeamIcon
            }
            if ($global:IsFontPickerPopupActive) {
                $script:FontPickerPopup.Icon = $TeamIcon
            }
        }
        else {
        ...
        if (Test-Path $AlliedIcon) {
            $DesktopAssistantForm.Icon = $AlliedIcon
            if ($global:IsCertCheckPopupActive) {
                $script:CertCheckPopup.Icon = $AlliedIcon
            }
            if ($global:IsLenderLFPPopupActive) {
                $script:LenderLFPPopup.Icon = $AlliedIcon
            }
            if ($global:IsBillingRestartPopupActive) {
                $script:BillingRestartPopup.Icon = $AlliedIcon
            }
            if ($global:IsHDTStoragePopupActive) {
                $script:HDTStoragePopup.Icon = $AlliedIcon
            }
            if ($global:IsFeedbackPopupActive) {
                $script:FeedbackPopup.Icon = $AlliedIcon
            }
            if ($global:IsThemeBuilderPopupActive) {
                $script:ThemeBuilderForm.Icon = $AlliedIcon
            }
            if ($global:IsColorPickerPopupActive) {
                $script:ColorPickerPopup.Icon = $AlliedIcon
            }
            if ($global:IsFontPickerPopupActive) {
                $script:FontPickerPopup.Icon = $AlliedIcon
            }
        }
        else {
...
# Function to send a cancel request to the AWS SSO login process
function Set-CancelLoginFlag {
    $script:cancelLoginRequested = $true
}
...
$StartupForm = New-WindowsForm -SizeX 475 -SizeY 400 -Text '' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'FixedSingle' -ControlBox $false -ShowInTaskbar $true -KeyPreview $false -MinimizeBox $false -Icon $TeamIcon
...
$HelpForm = New-WindowsForm -SizeX 200 -SizeY 200 -Text '' -StartPosition 'Manual' -TopMost $true -FormBorderStyle 'FixedSingle' -ControlBox $false -ShowInTaskbar $false -KeyPreview $false -MinimizeBox $false -Icon $null
...
$DesktopAssistantForm = New-WindowsForm -SizeX 1000 -SizeY 600 -Text 'ETG Desktop Assistant' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
...
$SubmitFeedback.add_Click({
    ...
    $script:FeedbackPopup = New-WindowsForm -SizeX 300 -SizeY 350 -Text 'Submit Feedback' -StartPosition 'CenterScreen' -TopMost $true -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $false -Icon $TeamIcon
    $global:IsFeedbackPopupActive = $true

    # Label for providing UserName
    $script:UserNameLabel = New-FormLabel -LocationX 20 -LocationY 25 -SizeX 200 -SizeY 20 -Text "Add Your Name?" -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign
    ...
    # UserName textbox
    $script:UserNameTextBox = New-FormTextBox -LocationX 20 -LocationY 100 -SizeX 170 -SizeY 20 -ScrollBars 'None' -Multiline $false -Enabled $false -ReadOnly $false -Text '' -Font $global:NormalFont

    # Label for feedback form
    $script:FeedbackLabel = New-FormLabel -LocationX 20 -LocationY 165 -SizeX 200 -SizeY 20 -Text 'Enter your feedback below:' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

    # Textbox for feedback form
    $script:FeedbackTextBox = New-FormTextBox -LocationX 20 -LocationY 185 -SizeX 240 -SizeY 50 -ScrollBars 'Vertical' -Multiline $true -Enabled $true -ReadOnly $false -Text '' -Font $global:NormalFont

    # Button for submitting feedback
    $script:SubmitFeedbackButton = New-FormButton -Text "Submit" -LocationX 20 -LocationY 260 -Width 100 -BackColor $global:DisabledBackColor -ForeColor $global:DisabledForeColor -Font $global:NormalFont -Enabled $false -Visible $true
...
$MenuThemeBuilder.add_Click({
    ...
    $script:ThemeBuilderForm = New-WindowsForm -SizeX 750 -SizeY 720 -Text 'Theme Builder' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
    ...
    $script:ThemeBuilderColorPickerButton.add_Click({
        ...
        $script:ColorPickerPopup = New-WindowsForm -SizeX 380 -SizeY 635 -Text 'Color Picker' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
        ...
        $script:ColorPickerPanel.Location = New-Object System.Drawing.Point(5, 40)
        $script:ColorPickerPanel.Size = New-Object System.Drawing.Size(370, 540)
        $script:ColorPickerPanel.AutoScroll = $true

        # Button for applying color choices
        $script:ColorPickerApplyButton = New-FormButton -Text "Apply Choices" -LocationX 50 -LocationY 10 -Width 100 -BackColor $ControlColor -ForeColor $ControlColorText -Font $global:NormalFont -Enabled $true -Visible $true

        # Button for clearing color choices
        $ColorPickerClearButton = New-FormButton -Text "Clear Choices" -LocationX 210 -LocationY 10 -Width 100 -BackColor $ControlColor -ForeColor $ControlColorText -Font $global:NormalFont -Enabled $true -Visible $true
    ...
    $script:ThemeBuilderSaveThemeButton.add_Click({
        ...
        if ($isValidBackColor -and $isValidForeColor -and $isValidAccentColor -and $isValidDisabledColor) {
            ...
            $script:ThemeBuilderSaveThemeForm = New-WindowsForm -SizeX 300 -SizeY 300 -Text 'Save Theme' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
            ...
            $script:ThemeBuilderThemeNameTextBox = New-FormTextBox -LocationX 20 -LocationY 40 -SizeX 150 -SizeY 20 -ScrollBars 'None' -Multiline $false -Enabled $true -ReadOnly $false -Text '' -Font $global:NormalFont

            # Label for theme name text box
            $script:ThemeBuilderThemeNameLabel = New-FormLabel -LocationX 20 -LocationY 20 -SizeX 150 -SizeY 20 -Text "Theme Name" -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

            # Text box for quote
            $script:ThemeBuilderQuoteTextBox = New-FormTextBox -LocationX 20 -LocationY 120 -SizeX 150 -SizeY 20 -ScrollBars 'None' -Multiline $false -Enabled $true -ReadOnly $false -Text '' -Font $global:NormalFont

            # Label for quote text box
            $script:ThemeBuilderQuoteLabel = New-FormLabel -LocationX 20 -LocationY 100 -SizeX 150 -SizeY 20 -Text "Quote" -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

            # Button for saving the theme
            $script:ThemeBuilderSaveThemePopupButton = New-FormButton -Text "Save Theme" -LocationX 20 -LocationY 180 -Width 100 -BackColor $ControlColor -ForeColor $ControlColorText -Font $global:NormalSmallFont -Enabled $false -Visible $true

            # Button for saving and applying the theme
            $script:ThemeBuilderSaveAndApplyThemeButton = New-FormButton -Text "Save && Apply" -LocationX 160 -LocationY 180 -Width 100 -BackColor $ControlColor -ForeColor $ControlColorText -Font $global:NormalSmallFont -Enabled $false -Visible $true
    ...
    $script:ThemeBuilderDeleteThemesButton.add_Click({
        ...
        $script:ThemeBuilderDeleteThemesForm = New-WindowsForm -SizeX 300 -SizeY 380 -Text 'Delete Custom Themes' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
        # Variable for showing popup is active is not needed since form is shown with ShowDialog()
        # This means the form will have focus while it's open, so the user can't interact with the main form
...
$MenuAboutItem.add_Click({
    ...
    $script:AboutForm = New-WindowsForm -SizeX 400 -SizeY 300 -Text 'About This App' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $false -Icon $TeamIcon
    ...
    $AboutLabel = New-FormLabel -LocationX 115 -LocationY 120 -SizeX 150 -SizeY 20 -Text 'ETG Desktop Assistant' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

    # Picture box for Allied Solutions logo
    $AboutMenuAlliedLogo = New-FormPictureBox -LocationX 50 -LocationY 5 -SizeX 275 -SizeY 100 -Image $AboutMenuAlliedLogoImage -SizeMode $ZoomSizeMode -Cursor $ArrowCursor

    # Label for version numbers
    $VersionLabel = New-FormLabel -LocationX 145 -LocationY 145 -SizeX 150 -SizeY 20 -Text "Version: 3.0.0" -Font $global:NormalFont -TextAlign $DefaultTextAlign

    # Label for upcoming features
    $UpcomingFeaturesLabel = New-FormLabel -LocationX 125 -LocationY 180 -SizeX 200 -SizeY 75 -Text "Upcoming Features`r`n• AWS Admin Tools`r`n• Event Viewer Wizard" -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

    # Label for Allied IMPACT
    $AlliedImpactLabel = New-FormLabel -LocationX 40 -LocationY 260 -SizeX 300 -SizeY 20 -Text "Made with Passion to make an IMPACT at Allied Solutions, LLC." -Font $global:ExtraSmallItalicBoldFont -TextAlign $DefaultTextAlign
...
$CertCheckWizardButton.add_Click({
    ...
    $script:CertCheckPopup = New-WindowsForm -SizeX 300 -SizeY 400 -Text 'Run SSL Cert Checks' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
    $global:IsCertCheckPopupActive = $true

    # Label for server choice radio buttons
    $script:ServerChoiceLabel = New-FormLabel -LocationX 20 -LocationY 20 -SizeX 200 -SizeY 20 -Text 'Choose servers to check:' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign
    ...
    $script:ServerChoicePanel.Location = New-Object System.Drawing.Size(20, 40)
    ...
    $script:CertCheckServerTextBox = New-FormTextBox -LocationX 20 -LocationY 100 -SizeX 150 -SizeY 50 -ScrollBars 'Vertical' -Multiline $true -Enabled $false -ReadOnly $false -Text '' -Font $global:NormalFont

    # Label for output radio buttons
    $script:OutputLabel = New-FormLabel -LocationX 20 -LocationY 165 -SizeX 200 -SizeY 20 -Text 'Output results to:' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign
    ...
    $script:ChooseTextFileButton = New-FormButton -Text "Browse" -LocationX 20 -LocationY 225 -Width 75 -BackColor $global:DisabledBackColor -ForeColor $global:DisabledForeColor -Font $global:NormalFont -Enabled $false -Visible $true

    # Panel for output radio buttons
    $script:OutputPanel = New-Object System.Windows.Forms.Panel
    $script:OutputPanel.Location = New-Object System.Drawing.Size(20, 185)
    ...
    $script:IgnoreNoResultsCheckBox = New-FormCheckbox -LocationX 20 -LocationY 270 -SizeX 200 -SizeY 20 -Text 'Ignore servers with no results?' -Font $global:NormalFont -Checked $false -Enabled $true

    # Button for running Cert Check
    $script:RunCertCheckButton = New-FormButton -Text "Run Checks" -LocationX 20 -LocationY 310 -Width 100 -BackColor $global:DisabledBackColor -ForeColor $global:DisabledForeColor -Font $global:NormalFont -Enabled $false -Visible $true
    ...
    $script:RunCertCheckButton.add_Click({
        ...
        if ($IncludeNoResultsCheckBox.Checked) {
...
$LaunchLFPWizardButton.Add_Click({
    ...
    $script:LenderLFPPopup = New-WindowsForm -SizeX 300 -SizeY 350 -Text 'Add Lender to LFP Services' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
...
$LaunchBillingRestartLabel = New-FormLabel -LocationX 397 -LocationY 90 -SizeX 143 -SizeY 20 -Text 'Restart Billing Services' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign
...
$LaunchBillingRestartButton.add_Click({
    ...
    $script:BillingRestartPopup = New-WindowsForm -SizeX 250 -SizeY 235 -Text 'Restart Billing File Services' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
    $global:IsBillingRestartPopupActive = $true

    # Combo box for selecting the environment where lender will be added
    $script:BillingRestartCombo = New-FormComboBox -LocationX 40 -LocationY 65 -SizeX 150 -SizeY 200 -Font $global:NormalFont
    @('QA', 'Staging', 'Production') | ForEach-Object { [void]$script:BillingRestartCombo.Items.Add($_) }

    # Label for Billing Restart combo box
    $script:BillingRestartComboLabel = New-FormLabel -LocationX 55 -LocationY 40 -SizeX 150 -SizeY 20 -Text 'Select Environment' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

    # Checkbox to confirm Production runs
    $script:BillingRestartProductionCheckbox = New-FormCheckbox -LocationX 25 -LocationY 115 -SizeX 200 -SizeY 20 -Text 'Check to confirm PRODUCTION run' -Font $global:ExtraSmallFont -Checked $false -Enabled $false

    # Button for restarting billing file services
    $script:BillingRestartButton = New-FormButton -Text "Restart" -LocationX 75 -LocationY 150 -Width 75 -BackColor $global:DisabledBackColor -ForeColor $global:DisabledForeColor -Font $global:NormalFont -Enabled $false -Visible $true
...
$LaunchHDTStorageButton.Add_Click({
    ...
    $script:HDTStoragePopup = New-WindowsForm -SizeX 300 -SizeY 350 -Text 'Create HDTStorage Table' -StartPosition 'CenterScreen' -TopMost $false -FormBorderStyle 'Fixed3D' -ControlBox $true -ShowInTaskbar $true -KeyPreview $true -MinimizeBox $true -Icon $TeamIcon
    $global:IsHDTStoragePopupActive = $true

    # Button for selecting the file to upload
    $script:HDTStorageFileButton = New-FormButton -Text "Browse" -LocationX 20 -LocationY 30 -Width 75 -BackColor $ControlColor -ForeColor $ControlColor -Font $global:NormalFont -Enabled $true -Visible $true

    # Label for the file location button
    $script:FileLocationLabel = New-FormLabel -LocationX 20 -LocationY 10 -SizeX 150 -SizeY 20 -Text 'File Location' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

    # Text box for entering the SQL instance
    $script:DBServerTextBox = New-FormTextBox -LocationX 20 -LocationY 80 -SizeX 200 -SizeY 20 -ScrollBars 'None' -Multiline $false -Enabled $true -ReadOnly $false -Text '' -Font $global:NormalFont

    # Label for the SQL instance text box
    $script:DBServerLabel = New-FormLabel -LocationX 20 -LocationY 60 -SizeX 150 -SizeY 20 -Text 'SQL Instance' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

    # Text box for entering the table name
    $script:TableNameTextBox = New-FormTextBox -LocationX 20 -LocationY 130 -SizeX 200 -SizeY 20 -ScrollBars 'None' -Multiline $false -Enabled $true -ReadOnly $false -Text '' -Font $global:NormalFont

    # Label for the table name text box
    $script:TableNameLabel = New-FormLabel -LocationX 20 -LocationY 110 -SizeX 150 -SizeY 20 -Text 'Table Name' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

    # Text box for entering secure password
    $script:SecurePasswordTextBox = New-FormTextBox -LocationX 20 -LocationY 180 -SizeX 200 -SizeY 20 -ScrollBars 'None' -Multiline $false -Enabled $true -ReadOnly $false -Text '' -Font $global:NormalFont
    $script:SecurePasswordTextBox.PasswordChar = '*'

    # Label for the secure password text box
    $script:SecurePasswordLabel = New-FormLabel -LocationX 20 -LocationY 160 -SizeX 150 -SizeY 20 -Text 'Your Allied Password' -Font $global:NormalBoldFont -TextAlign $DefaultTextAlign

    # Checkbox to use the user's PW entered in Password Manager
    $script:HDTStoragePWCheckbox = New-FormCheckbox -LocationX 20 -LocationY 210 -SizeX 200 -SizeY 20 -Text 'Use Your Saved PW?' -Font $global:NormalFont -Checked $false -Enabled (-not [string]::IsNullOrEmpty($global:SecurePW))

    # Button for running the script to create the HDTStorage table
    $script:CreateHDTStorageButton = New-FormButton -Text "Create" -LocationX 20 -LocationY 250 -Width 75 -BackColor $global:DisabledBackColor -ForeColor $global:DisabledForeColor -Font $global:NormalFont -Enabled $false -Visible $true
